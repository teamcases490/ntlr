...
def compute_ntlr(current_df, hist_df):
    ...
    temporal_score = np.clip(slope * mean_500, -75, 150) # change this cap limit on the basis of distribuiton of slope*mean_500 values
    # temporal_score = 50 * np.tanh((slope * mean_500) / 100) # alternative with tanh to reduce impact of outliers
...
def main():
    ...
    for row in tqdm(df_input.itertuples(index=False), total=len(df_input), desc="Processing Locations"):
        lat = row.Latitude
        lon = row.Longitude
